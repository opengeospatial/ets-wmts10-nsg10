...
public class GetCapabilitiesWellKnownScaleTest extends AbstractBaseGetCapabilitiesFixture {
	...
	/**
	 * --- NSG Requirement 13: An NSG WMTS server shall employ the Well-Known Scale Sets
	 * identified in Annex B (based upon World Mercator projection EPSG 3395 and WGS 84
	 * Geodetic EPSG 4326) ---
	 */
	...
	@Test(description = "NSG Web Map Tile Service (WMTS) 1.0.0, Requirement 13",
	...
	@Test(description = "NSG Web Map Tile Service (WMTS) 1.0.0, Requirement 13, Annex B.1",
	...
	@Test(description = "NSG Web Map Tile Service (WMTS) 1.0.0, Requirement 13, Annex B.2",
			dependsOnMethods = "wmtsCapabilitiesExists")
	...
	@Test(description = "NSG Web Map Tile Service (WMTS) 1.0.0, Requirement 13, Annex B.3",
			dependsOnMethods = "wmtsCapabilitiesExists")
	...
	private void testWellKnownScale(String wellKnownScaleSet, NodeList listFromAnnexB) throws XPathExpressionException {
		...
		List<Element> tileMatrixSetsWithCrs = retrieveTileMatrixSetsWithSupportedCrs(wellKnownScaleSet,
				tileMatrixSetList);
		assertTrue(tileMatrixSetsWithCrs.size() > 0,
				"Well-Known Scale Set for " + wellKnownScaleSet + " is not advertised in WMTS");
		...
		assertTrue(isAtLeastOneTileMatrixSetWithCrsWellKnown,
				"Scale Set for " + wellKnownScaleSet + " is advertised in WMTS but not Well-Known");
	}
	...
	private boolean isTileMatrixSetWellKnown(NodeList listFromAnnexB, Element tileMatrixSet) {
		...
		// -- check each advertised zoom level, ensuring each matches the prescribed
		// tables
