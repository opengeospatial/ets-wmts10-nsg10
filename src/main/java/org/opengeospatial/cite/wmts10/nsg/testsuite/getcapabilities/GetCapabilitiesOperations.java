...
public class GetCapabilitiesOperations extends AbstractBaseGetCapabilitiesFixture {
	...
	/**
	 * --- NSG Requirement 2: An NSG WMTS server shall declare its support for
	 * GetCapabilities operations using KVP with HTTP GET by providing an
	 * OperationsMetadata section in the ServiceMetadata document with an Operation
	 * section for each supported HTTP request type. ---
	 */
	@Test(description = "NSG Web Map Tile Service (WMTS) 1.0.0, Requirement 2",
	...
	@Test(description = "NSG Web Map Tile Service (WMTS) 1.0.0, Requirement 2",
			dependsOnMethods = "wmtsCapabilitiesOperationsMetadataExists")
	...
	@Test(description = "NSG Web Map Tile Service (WMTS) 1.0.0, Requirement 2",
			dependsOnMethods = "wmtsCapabilitiesOperationsMetadataOperationExists")
	public void wmtsCapabilitiesKVPRequestsExists() {
		URI uri = ServiceMetadataUtils.getOperationEndpoint_KVP(this.wmtsCapabilities, WMTS_Constants.GET_CAPABILITIES,
				ProtocolBinding.GET);
		assertTrue(uri != null,
	...
	@Test(description = "NSG Web Map Tile Service (WMTS) 1.0.0, Requirement 2",
			dependsOnMethods = "wmtsCapabilitiesKVPRequestsExists")
