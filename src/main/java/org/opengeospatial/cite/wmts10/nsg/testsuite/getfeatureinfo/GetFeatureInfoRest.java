...
/*---
	NSG Requirement 10: 
		An NSG WMTS server shall provide standard endpoints from which representation of the GetFeatureInfo resources 
		can be obtained. 	
---*/
	...
	{
		getFeatureInfoURI = ServiceMetadataUtils.getOperationEndpoint_REST(wmtsCapabilities, WMTS_Constants.GET_FEATURE_INFO, ProtocolBinding.GET);
		
		assertTrue(getFeatureInfoURI != null,  "GetFeatureInfo (GET) endpoint not found or REST is not supported in ServiceMetadata capabilities document.");
	}	
	...
	//	if ( getFeatureInfoURI == null )
        ...
        assertTrue(WMTS_Constants.GET_FeatureInfo.equals( responseDoc.getDocumentElement().getLocalName() ),
			...
			// --- Example of invalid URL
			{
				...
				Assert.assertFalse( rsp.getStatus() == 200, "Expected status code from Invalid REST GetFeatureInfo request is not expected to be 200. " );
