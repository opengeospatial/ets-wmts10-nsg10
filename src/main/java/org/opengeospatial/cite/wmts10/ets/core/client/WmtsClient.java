...
public class WmtsClient {
	...
	/**
	 * Constructs a client that is aware of the capabilities of some WMTS. The request and
	 * response may be logged to a default JDK logger (in the namespace
	 * "com.sun.jersey.api.client").
	 * @param wmtsCapabilities the WMTS capabilities document, never <code>null</code>
	 */
	...
	/**
	 * Retrieves a complete representation of the capabilities document from the WMTS
	 * implementation described by the service metadata.
	 ...
	 * @param request the KVP encoded request, never <code>null</code>
	 * @param endpoint the service endpoint, never <code>null</code>
	...
	private Response submitPostRequest(WebTarget target, Source payload) {
		...
		try {
			response = target.request(WMTS_Constants.SOAP_XML)
				.buildPost(Entity.entity(payload, WMTS_Constants.SOAP_XML))
				.invoke();
