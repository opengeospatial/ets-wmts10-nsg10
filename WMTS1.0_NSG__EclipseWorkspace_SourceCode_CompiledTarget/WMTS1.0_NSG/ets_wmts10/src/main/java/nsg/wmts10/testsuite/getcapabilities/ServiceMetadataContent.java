...
import static nsg.wmts10.testsuite.getcapabilities.GetCapabilitiesKeywordTest.verifyNASkeywords;
...
---*/
	...
	/*--
	@Test(description = "NSG Web Map Tile Service (WMTS) 1.0.0, Requirements 4,7", dependsOnMethods="wmtsCapabilitiesExists")
	public void wmtsCapabilitiesThemesExists()
	{
		// --- Test Method: 1 (The response has all required service metadata elements) - Themes are optional
		assertXPath("//wmts:Themes",               wmtsCapabilities, NS_BINDINGS );		
	}
	--*/
	...
	{
		// --- Test Method: 3 (The response in the Abstract element contains the following information: "This service implements the NSG WMTS 1.0.0 profile version 1.0.")
		XPath xPath = createXPath();
		Node abstractElement = (Node)xPath.evaluate("//ows:ServiceIdentification/ows:Abstract", wmtsCapabilities, XPathConstants.NODE);
		...
		assertTrue(((abstractStatement != null) && !abstractStatement.isEmpty()), "The <Abstract> statement is blank.");
		assertTrue(abstractStatement.contains(NSG_ABSTRACTSTATEMENT), 
				"The <Abstract> statement (" +  abstractStatement + ") should be:  "+ NSG_ABSTRACTSTATEMENT + ".");
	}
	...
	{
		// --- Test Method: 4.0 - (The response provides keywords based upon the NAS for the following: Layer data structure, Style data structure, Dimension data structure, TileMatrixSet data structure, TileMatrix data structure, and Themes data structure.)
		verifyNASkeywords(false, "WMTS ServiceMetadata Capabilities docuemnt", wmtsCapabilities, "//ows:ServiceIdentification/ows:Keywords");
	}
	...
	{
		// --- Test Method: 4.1 - (The response provides keywords based upon the NAS for the following: Layer data structure ...)
	...
	{
		// --- Test Method: 4.2 - (The response provides keywords based upon the NAS for the following: ..., Style data structure, ...)
	...
	{
		// --- Test Method: 4.3 - (The response provides keywords based upon the NAS for the following: ..., Dimension data structure, ...)
		verifyNASkeywords(false, "Dimension Data Structure", wmtsCapabilities, "//wmts:Layer/wmts:Dimension/ows:Keywords");
	}
	...
	{
		// --- Test Method: 4.4 - (The response provides keywords based upon the NAS for the following: ... TileMatrixSet data structure, ...)
	...
	{
		// --- Test Method: 4.5 - (The response provides keywords based upon the NAS for the following: ... TileMatrix data structure ...)
	...
	{
		// --- Test Method: 4.6 - (The response provides keywords based upon the NAS for the following: ... Themes data structure.)
			...
			throws XPathExpressionException, XPathFactoryConfigurationException
	{
		...
		for (int i=0; i<layerInfo.size(); i++)
		{
			...
			// --- will soft assess in order to go thru all layers // assertXPath(exprPath, wmtsCapabilities, NS_BINDINGS);
	...
	public void wmtsCapabilitiesLayerStyleLegends() 
			throws XPathExpressionException, XPathFactoryConfigurationException
	{
		// --- Test Method: 7 (The response provides an associated legend in at least one of the following formats: PNG, GIF, JPEG)
		...
		// --- Test Method: 12 (The provided <MinScaleDenominator> value is less than or equal to the <MaxScaleDenominator>)
		XPath xPath = createXPath();
		if ((layerInfo == null ) || (layerInfo.size() <= 0 ))
		{
			throw new SkipException("There are no Layers identified" );
				...
				for ( int si=0; si<layerStyles.getLength(); si++)
				{
					...
					if (( legendList != null ) && (legendList.getLength() > 0 ))
					{
						...
						for (int li=0; li<legendList.getLength(); li++)
						{
							...
							sa.assertTrue((!Strings.isNullOrEmpty(format)) && (!Strings.isNullOrEmpty(url)), "Legend for Style: " + styleIdentifier + " under Layer: " + layer.getLayerName() + " is not properly defined.");

							// -- Test for formats (Test Method 7)
							foundPreferredFormat |= ( format.equals(WMTS_Constants.IMAGE_PNG) || 
													  format.equals(WMTS_Constants.IMAGE_GIF) ||
													  format.equals(WMTS_Constants.IMAGE_JPEG));
							...
							sa.assertTrue(!Strings.isNullOrEmpty(minScaleDenominator), 
									"<Layer>: " + layer.getLayerName() + " does not have a minScaleDenominator for <Style>: " + styleIdentifier + " for a Legend");
							sa.assertTrue(!Strings.isNullOrEmpty(maxScaleDenominator), 
									"<Layer>: " + layer.getLayerName() + " does not have a maxScaleDenominator for <Style>: " + styleIdentifier + " for a Legend");
							
							// -- Test for mix/max (Test Method 12)
							if ( !Strings.isNullOrEmpty(minScaleDenominator) && !Strings.isNullOrEmpty(maxScaleDenominator))
							{
								...
								sa.assertTrue(minScale <= maxScale,
										"The minScaleDenominator > maxScaleDenominator for <Layer>: " + layer.getLayerName() + " and <Style>: " + styleIdentifier + " for a Legend");
							}
						}
						sa.assertTrue(foundPreferredFormat, 
								"<Style>: " + styleIdentifier + " under <Layer>: " + layer.getLayerName() + " does not use a preferred Legend image format.");
					}
				}			
			}
		}
...
//			sa.assertTrue((layerDimension != null) && ( layerDimension.getLength()>0), "There are no <Dimension> elements for <Layer>:  " + layer.getLayerName());
			...
			if (( layerDimension != null ) && ( layerDimension.getLength() > 0 ))
			{
				...
				for ( int si=0; si<layerDimension.getLength(); si++)
				{
					...
					sa.assertTrue(!Strings.isNullOrEmpty(dimensionIdentifier), "There is no <Identifier> element for the <Dimension>, under <Layer>: " + layer.getLayerName());
					if ( !Strings.isNullOrEmpty(dimensionIdentifier))
					{
						sa.assertTrue(!Strings.isNullOrEmpty(defaultValue), "There is no <Default> value element for the <Dimension>, under <Layer>: " + layer.getLayerName());
	...
	public void wmtsCapabilitiesLayerInfoFormat() 
			throws XPathExpressionException, XPathFactoryConfigurationException
	{
		// --- Test Method: 10 (The response provides a valid output format for Layer data structure infoFormat parameter thus enabling GetFeatureInfo)
		XPath xPath = createXPath();
		if ((layerInfo == null ) || (layerInfo.size() <= 0 ))
		{
			throw new SkipException("There are no Layers identified" );
		...
		for (int i=0; i<layerInfo.size(); i++)
		{
			...
			if (( layerInfoFormats != null ) && ( layerInfoFormats.getLength() > 0 ))
			{
				for ( int fi=0; fi< layerInfoFormats.getLength(); fi++)
				{
					...
					sa.assertTrue(!Strings.isNullOrEmpty(infoFormat), "There is no or invalid <InfoFormat> element under <Layer>: " + layer.getLayerName());	
				}			
			}
		}
		sa.assertAll();
